...
function Server(compiler, options) {
    ...
    this.fallbackBase = options.fallbackBase;
    ...
    //app.configure(function() {
        ...
        app.use(this.middleware = options.middleware || webpackDevMiddleware(compiler, options));
    //}.bind(this));
        ...
        fs.stat(target, function(err, stat) {
            ...
            if(!err && stat.isFile()) {
                ...
                if (this.fallbackBase) {
                    // go to fallback
                    var targetFallback = path.join(this.fallbackBase, _path);
                    //console.log('fallback', targetFallback);
                    fs.stat(targetFallback, function(err, stat) {
                        if(!err && stat.isFile()) {
                            res.setHeader("Content-Type", mime.lookup(_path));
                            fs.createReadStream(targetFallback).pipe(res);
                        } else if(!err && stat.isDirectory()) {
                            res.writeHead(302, {
                                'Location': req.path + (/\/$/.test(_path) || !_path ? "index.html" : "/") + (req._parsedUrl.search || "")
                            });
